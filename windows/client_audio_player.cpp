...
ClientAudioPlayer::ClientAudioPlayer(const WCHAR *url, shared_ptr<MethodChannel<EncodableValue>> channel, const char *playerId) : m_cRef(1) {
...
void ClientAudioPlayer::OnMediaPlayerEvent(MFP_EVENT_HEADER *pEventHeader) {
  switch (pEventHeader->eEventType) {
    ...
    case MFP_EVENT_TYPE_MEDIAITEM_SET: {
      ...
      dispatchEvent(ClientPlayerEvent::Prepared, &EncodableMap({{EncodableValue("duration"), EncodableValue(toInteger(time.hVal)/ 10000)}}));
    } break;
