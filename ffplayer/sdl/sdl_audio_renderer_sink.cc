...
void SdlAudioRendererSink::Initialize(int wanted_nb_channels,
                                      int wanted_sample_rate,
                                      RenderCallback *render_callback) {
  DCHECK(render_callback);
  render_callback_ = render_callback;

  hw_audio_buffer_size_ =
      OpenAudioDevices(wanted_nb_channels, wanted_sample_rate);
...
bool SdlAudioRendererSink::SetVolume(double volume) { return false; }

int SdlAudioRendererSink::OpenAudioDevices(int wanted_nb_channels,
                                           int wanted_sample_rate) {
  ...
  wanted_spec.samples =
      std::max(kSdlAudioMinBufferSize,
  ...
  wanted_spec.callback = [](void *userdata, Uint8 *stream, int len) {
    ...
    DLOG_IF(INFO, (av_gettime_relative() - start) > 2000)
        << "render consume time more than 2ms: "
        << double(av_gettime_relative() - start) / 1000.0 << "ms";
  };
