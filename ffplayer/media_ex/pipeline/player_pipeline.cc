...
PlayerPipeline::PlayerPipeline() {
  pipeline_runner_ = std::shared_ptr<MessageLoop>(MessageLoop::prepare_looper("media_pipeline"));
}

PlayerPipeline::~PlayerPipeline() {
  pipeline_runner_->Quit();
}

void PlayerPipeline::Open(const std::string &url, PlayerPipeline::OpenSourceCB open_source_cb) {
...
bool PlayerPipeline::OpenTask(const std::string &url) {
  ...
  decoder_runner_ = std::shared_ptr<MessageLoop>(MessageLoop::prepare_looper("decoder"));
  demuxer_ = std::make_shared<Demuxer>(decoder_runner_, data_source_.get(),
