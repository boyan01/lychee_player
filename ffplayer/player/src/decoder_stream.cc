...
void DecoderStream<StreamType>::Initialize(
    DemuxerStream *stream, DecoderStream::InitCallback init_callback) {
  decoder_ = std::make_unique<Decoder>();
  demuxer_stream_ = stream;

  traits_->InitializeDecoder(
      decoder_.get(), stream,
      bind_weak(&DecoderStream<StreamType>::OnFrameAvailable,
                this->shared_from_this()));
  auto init_callback_bound =
      BindToRunner(task_runner_.get(), std::move(init_callback));
  init_callback_bound(true);
}

template <DemuxerStream::Type StreamType>
void DecoderStream<StreamType>::Read(
    DecoderStream::ReadCallback read_callback) {
  ...
  task_runner_->PostTask(
      FROM_HERE, bind_weak(&DecoderStream<StreamType>::ReadFromDemuxerStream,
                           this->shared_from_this()));
}
...
void DecoderStream<StreamType>::ReadFromDemuxerStream() {
  ...
  if (CanDecodeMore() && !reading_demuxer_stream_) {
    ...
    demuxer_stream_->Read(std::bind(&DecoderStream<StreamType>::OnBufferReady,
...
void DecoderStream<StreamType>::OnBufferReady(
  ...
  task_runner_->PostTask(FROM_HERE,
                         [weak_this(std::weak_ptr<DecoderStream<StreamType>>(
                              this->shared_from_this())),
                          buffer]() {
...
void DecoderStream<StreamType>::DecodeTask(
    std::shared_ptr<DecoderBuffer> decoder_buffer) {
  DCHECK(task_runner_->BelongsToCurrentThread());

  if (decoder_buffer->end_of_stream()) {
    DLOG(WARNING) << "an end stream decode buffer";
    //    decode_task_runner_->PostTask(FROM_HERE,
    //                           bind_weak(&DecoderStream<StreamType>::ReadFromDemuxerStream,
    //                           this->shared_from_this()));
    // TODO
    return;
  }
...
bool DecoderStream<StreamType>::CanDecodeMore() {
  DCHECK(task_runner_->BelongsToCurrentThread());
  return pending_decode_requests_ < GetMaxDecodeRequests() &&
         outputs_.size() <= 9;
}

template <DemuxerStream::Type StreamType>
void DecoderStream<StreamType>::OnFrameAvailable(
    std::shared_ptr<Output> output) {
  DCHECK(task_runner_->BelongsToCurrentThread());

  DLOG_IF(WARNING, outputs_.size() > 9)
      << "outputs is full enough. " << outputs_.size();
...
void DecoderStream<StreamType>::Flush() {
  task_runner_->PostTask(FROM_HERE, [this] { decoder_->Flush(); });
  outputs_.clear();
}
