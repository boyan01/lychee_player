...
int AudioDecoder::Initialize(const AudioDecodeConfig &config,
  ...
  auto ret = avcodec_parameters_to_context(codec_context_.get(),
                                           &config.codec_parameters());
  DCHECK_GE(ret, 0);
  ...
  DCHECK_GE(ret, 0) << "can not open avcodec, reason: "
...
void AudioDecoder::Decode(
    const std::shared_ptr<DecoderBuffer> &decoder_buffer) {
  DCHECK(stream_);
  DCHECK(ffmpeg_decoding_loop_);

  switch (ffmpeg_decoding_loop_->DecodePacket(
      decoder_buffer->av_packet(), std::bind(&AudioDecoder::OnFrameAvailable,
...
bool AudioDecoder::OnFrameAvailable(AVFrame *frame) {
  ...
  if (swr_ctx_) {
    ...
    int out_count = frame->nb_samples *
                        audio_decode_config_.samples_per_second() /
                        frame->sample_rate +
                    256;
    int out_size =
        av_samples_get_buffer_size(nullptr, audio_decode_config_.channels(),
    ...
    auto out_nb_samples =
        swr_convert(swr_ctx_, &data, out_size, in, frame->nb_samples);
    if (out_nb_samples < 0) {
  ...
  double pts =
      frame->pts == AV_NOPTS_VALUE
          ? NAN
          : av_q2d(audio_decode_config_.time_base()) * double(frame->pts);
  output_callback_(std::make_shared<AudioBuffer>(
      data, data_size, pts, audio_device_info_.bytes_per_sec));
  return true;
}

// static
int64 AudioDecoder::GetChannelLayout(AVFrame *frame) {
  bool valid = frame->channel_layout &&
               frame->channels ==
                   av_get_channel_layout_nb_channels(frame->channel_layout);
  return valid ? int64(frame->channel_layout)
               : av_get_default_channel_layout(frame->channels);
}

void AudioDecoder::Flush() { avcodec_flush_buffers(codec_context_.get()); }
