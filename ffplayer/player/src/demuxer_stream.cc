...
std::shared_ptr<DemuxerStream> DemuxerStream::Create(
    media::Demuxer* demuxer, AVStream* stream,
  ...
  switch (stream->codecpar->codec_type) {
    ...
    case AVMEDIA_TYPE_VIDEO: {
      type = Video;
      video_decode_config = std::make_unique<VideoDecodeConfig>(
          *stream->codecpar, stream->time_base,
          av_guess_frame_rate(format_context, stream, nullptr), 10);
      break;
    }
    default:
      type = UNKNOWN;
...
void DemuxerStream::Read(ReadCallback read_callback) {
  ...
  task_runner_.PostTask(
      FROM_HERE, [this, capture0 = BindToCurrentLoop(
...
void DemuxerStream::Abort() {
  ...
  task_runner_.PostTask(FROM_HERE,
                        std::bind(&DemuxerStream::SatisfyPendingRead, this));
}
