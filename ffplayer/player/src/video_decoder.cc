...
enum AVPixelFormat lychee_avcodec_get_format(struct AVCodecContext *avctx,
  ...
  if (avctx->hw_device_ctx && avctx->codec->hw_configs) {
    ...
    for (i = 0;; i++) {
      ...
      for (n = 0; fmt[n] != AV_PIX_FMT_NONE; n++) {
        if (config->pix_fmt == fmt[n]) return fmt[n];
      }
    }
  }
  ...
  if (!(desc->flags & AV_PIX_FMT_FLAG_HWACCEL)) return fmt[n - 1];
...
int VideoDecoder::Initialize(VideoDecodeConfig config, DemuxerStream *stream,
  ...
  DCHECK(codec) << "no decoder could be found"
  ...
  DCHECK_LE(stream_lower, int(codec->max_lowres))
      << "The maximum value for lowres supported by the decoder is "
      << codec->max_lowres << ", but is " << int(codec->max_lowres);
  ...
  ret = av_hwdevice_ctx_create(&hw_device_context_,
                               AVHWDeviceType::AV_HWDEVICE_TYPE_VIDEOTOOLBOX,
                               nullptr, nullptr, 0);
  if (ret < 0) {
    DLOG(INFO) << "failed to create hw device_context"
...
void VideoDecoder::Decode(std::shared_ptr<DecoderBuffer> decoder_buffer) {
  ...
  switch (ffmpeg_decoding_loop_->DecodePacket(
      decoder_buffer->av_packet(), std::bind(&VideoDecoder::OnFrameAvailable,
...
bool VideoDecoder::OnFrameAvailable(AVFrame *frame) {
  ...
  auto pts =
      (frame->pts == AV_NOPTS_VALUE)
          ? NAN
          : double(frame->pts) * av_q2d(video_decode_config_.time_base());

  std::shared_ptr<VideoFrame> video_frame =
      std::make_shared<VideoFrame>(frame, pts, duration, 0);
  output_callback_(std::move(video_frame));
  return false;
}

void VideoDecoder::Flush() { avcodec_flush_buffers(codec_context_.get()); }
