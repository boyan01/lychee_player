...
class MockProtocol : public FFmpegUrlProtocol {
 public:
  ...
  explicit MockProtocol(bool track_destroy = true) : track_destroy_(track_destroy) {}
...
class FFmpegGlueTest : public ::testing::Test {
 ...
 public:
  ...
  FFmpegGlueTest() : protocol_(nullptr) {}
  ...
  virtual void OpenContext(MockProtocol *protocol, URLContext *context) {
    ...
    EXPECT_CALL(*protocol, IsStreaming()).WillOnce(testing::Return(true));
...
TEST_F(FFmpegGlueTest, AddRemoveGetProtocol) {
  ...
  auto protocol_a = std::make_unique<testing::StrictMock<MockProtocol>>();
  auto protocol_b = std::make_unique<testing::StrictMock<MockProtocol>>();
...
TEST_F(FFmpegGlueTest, OpenClose) {
  ...
  auto protocol = std::make_unique<StrictMock<MockProtocol>>();
...
TEST_F(FFmpegGlueTest, Write) {
  auto protocol = std::make_unique<StrictMock<MockProtocol>>(false);
  URLContext context;
  ...
  EXPECT_EQ(AVERROR(EIO), protocol_->url_write(&context, nullptr, 0));
