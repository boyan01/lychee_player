...
class DemuxerTest : public testing::Test {
 ...
 protected:
  ...
  testing::MockFunction<void(std::unique_ptr<MediaTracks>)> media_tracks_update_callback_;
  ...
  void CreateDemuxer(const std::string &name) {
    ...
    demuxer_ = std::make_shared<Demuxer>(
        task_runner,
        name,
        media_tracks_update_callback_.AsStdFunction()
    );
  }
  ...
  void InitializeDemuxer() {
    ...
    EXPECT_CALL(status_cb_, Call(testing::_)).Times(1).WillOnce([&](int status) {
      EXPECT_EQ(status, 0);
