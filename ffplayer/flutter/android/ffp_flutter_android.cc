...
FlutterAndroidVideoRender::TextureRegistry
    FlutterAndroidVideoRender::flutter_texture_registry;
...
void FlutterAndroidVideoRender::RenderPicture(Frame& frame) {
  ...
  if (ANativeWindow_lock(texture_->native_window(), &window_buffer_, nullptr) <
      0) {
    av_log(nullptr, AV_LOG_WARNING,
           "FlutterAndroidVideoRender lock window failed.\n");
    return;
  }

  img_convert_ctx_ = sws_getCachedContext(
      img_convert_ctx_, frame.width, frame.height, AVPixelFormat(frame.format),
  ...
  av_log(nullptr, AV_LOG_INFO,
         "render_frame(%lld, %p): render first pixel = 0x%2x%2x%2x%2x \n",
         texture_->id(), texture_.get(), buffer[3], buffer[0], buffer[1],
         buffer[2]);
#endif
...
FlutterTextureEntry::FlutterTextureEntry(JNIEnv* env, jobject texture)
  ...
  jmethodID get_surface =
      env->GetMethodID(clazz, "getSurface", "()Landroid/view/Surface;");
  ...
  ANativeWindow_setBuffersGeometry(native_window_, 1280, 720,
