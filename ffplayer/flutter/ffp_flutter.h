...
FFPLAYER_EXPORT void media_set_play_when_ready(MediaPlayer *player,
                                               bool play_when_ready);

FFPLAYER_EXPORT double ffplayer_get_current_position(CPlayer *player);

FFPLAYER_EXPORT void ffplayer_seek_to_position(CPlayer *player,
                                               double position);
...
/**
 * @return -1 if player invalid; -2 if render_context invalid; 0 if none picture
 * rendered.
 */
FFPLAYER_EXPORT double ffp_get_video_aspect_ratio(CPlayer *player);

/**
 * Set Message callback for dart. Post message to dart isolate by @param
 * send_port.
 */
FFPLAYER_EXPORT void ffp_set_message_callback_dart(CPlayer *player,
                                                   int64_t send_port);
