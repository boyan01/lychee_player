...
static void quit_message_loop_with_wait(
    std::shared_ptr<MessageLooper> message_loop) {
  message_loop.reset();
  std::this_thread::sleep_for(std::chrono::milliseconds(200));
}
...
message_loop->PostTask(FROM_HERE, [&]() {
  message_loop->PostTask(
      FROM_HERE_WITH_EXPLICIT_FUNCTION("PostNestedTask#lambda"),
      task.AsStdFunction());
});
TEST_MESSAGE_LOOP_END()
...
TEST_F(MessageLoopTest, BindTo) {
  ...
  message_loop_->PostTask(FROM_HERE, [&] {
    auto function1_bound =
        media::BindToLoop(message_loop_, function1.AsStdFunction());
    function1_bound(0);
    function2.AsStdFunction()(0);
  });
