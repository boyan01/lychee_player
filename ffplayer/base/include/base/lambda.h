...
class WeakBind {
 ...
 public:
  ...
  WeakBind(const std::function<Result(Class *, Args...)> &callback,
           const std::weak_ptr<Class> &weak_ptr,
           Result default_value) : callback_(callback), weak_ptr_(weak_ptr), default_value_(default_value) {
...
template<class Result, class Class, class ...Args>
WeakBind<Result, Class, Args...> bind_weak(
    Result(Class::*callback)(Args...),
    std::shared_ptr<Class> ptr,
